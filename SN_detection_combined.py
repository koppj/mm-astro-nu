"A simulation of the combined detection of supernova neutrinos at DUNE and at Hyper-K assuming coherent conversion"
import matplotlib
...
n_ar = 40e9*mol/39.95    #number of Argon atoms in 40kt
n_e = 40e9*mol*18/39.95    #number of electrons in 40kt of liquid Argon

binning = np.linspace(-5e-3, 2e-2, 6)

def spec0 (x, mean, alph):
    "Energy spectrum of SN neutrinos without magnetic moment mu in MeV"
    return (x**alph * np.exp(-(alph+1)*x/mean)
            * ((1+alph)/mean)**(1+alph)/special.gamma(1+alph))

def spec (x, mean, alph, mu, mN, mnu):
    "Energy spectrum of SN neutrinos with magnetic moment mu in MeV after conversion"
    return (x**alph * np.exp(-(alph+1)*x/mean)
            * ((1+alph)/mean)**(1+alph)/special.gamma(1+alph)
            * ((mN**2-mnu**2)**2 + 2 * x**2 * mu**2 * B**2 * MeV**(-2) * (1 +
                np.cos(np.sqrt((mN**2-mnu**2)**2*MeV**2/x**2+4*mu**2*B**2)*T/(2*hbar))
                * np.exp(-5e-1*(mN**2-mnu**2)**2*MeV**2*T/(2*np.pi*np.sqrt((mN**2-mnu**2)**2*MeV**2/x**2+4*mu**2*B**2)*2*hbar*x**3)
                         - 1e-11*2*B*mu**2*T/(np.sqrt((mN**2-mnu**2)**2*MeV**2/x**2+4*mu**2*B**2)*2*hbar))))
            /((mN**2-mnu**2)**2 + 4 * x**2 * mu**2 * B**2 * MeV**(-2)))

def frac_hk(mean, alph, thrsh, mu, mN, mnu):
    "Convoluted flux above the detection energy threshold with the energy"
    if mean == 0:
        return 0
    elif mu == 0 or np.abs(mN**2-mnu**2) > np.abs(1e3*2*mean*mu*B/MeV):
        return mean * special.gammaincc(2+alph, thrs*(1+alph)/mean)
    else:
        return integrate.quad(lambda x: spec(x, mean, alph, mu, mN, mnu) * x, thrsh, np.infty, args=(), limit=100)[0]

def frac_dn(mean, alph, mu, mN, mnu, sigma):
    "Convoluted flux above the detection energy threshold with the cross-section"
    if mean == 0:
        return 0
    elif mu == 0 or np.abs(mN**2-mnu**2) > np.abs(1e3*2*mean*mu*B/MeV):
        return integrate.quad(lambda x: spec0(x, mean, alph) * sigma_extract(sigma, x),
                              0, 100, args=(), limit=100)[0]
    else:
        return integrate.quad(lambda x: spec(x, mean, alph, mu, mN, mnu) * sigma_extract(sigma, x),
                              0, 100, args=(), limit=100)[0]
...
def chi_square (mu, mN, mh, hk=True, dn=True):
    ...
    if hk:
        ...
        frac_e_hk = [frac_hk(nu_e[2][i], nu_e[3][i], thrs, 0, 0, 0) for i in range(idx_in, idx_fn+1)]
        frac_bare_hk = [frac_hk(nubar_e[2][i], nubar_e[3][i], thrs, 0, 0, 0) for i in range(idx_in, idx_fn+1)]
        frac_x_hk = [frac_hk(nu_x[2][i], nu_x[3][i], thrs, 0, 0, 0) for i in range(idx_in, idx_fn+1)]

        # With magnetic conversion
        frac_emu_hk = [frac_hk(nu_e[2][i], nu_e[3][i], thrs, U[1,i_e-1]*mu, mN, m[i_e-1]) for i in range(idx_in, idx_fn+1)]
        frac_baremu_hk = [frac_hk(nubar_e[2][i], nubar_e[3][i], thrs, U[1,i_be-1]*mu, mN, m[i_be-1]) for i in range(idx_in, idx_fn+1)]
        frac_xmu_hk = [[frac_hk(nu_x[2][i], nu_x[3][i], thrs, U[1,j]*mu, mN, m[j]) for i in range(idx_in, idx_fn+1)] for j in range(3)]
    ...
    if dn:
        ...
        frac_e_dn = [[[frac_dn(nu_e[2][i], nu_e[3][i], 0, 0, 0, sigma_dn[j][l]) for i in range(idx_in, idx_fn+1)]
                      for l in range(2)] for j in range(2)]
        frac_bare_dn = [[[frac_dn(nubar_e[2][i], nubar_e[3][i], 0, 0, 0, sigma_bar_dn[j][l]) for i in range(idx_in, idx_fn+1)]
                         for l in range(2)] for j in range(2)]
        frac_x_dn = [[[frac_dn(nu_x[2][i], nu_x[3][i], 0, 0, 0, sigma_dn[j][l]) for i in range(idx_in, idx_fn+1)]
                      for l in range(2)] for j in range(2)]
        frac_barx_dn = [[[frac_dn(nu_x[2][i], nu_x[3][i], 0, 0, 0, sigma_bar_dn[j][l])
                          for i in range(idx_in, idx_fn+1)] for l in range(2)] for j in range(2)]

        # With magnetic conversion
        frac_emu_dn = [[[frac_dn(nu_e[2][i], nu_e[3][i], U[1,i_e-1]*mu, mN, m[i_e-1], sigma_dn[j][l]) for i in range(idx_in, idx_fn+1)]
                     for l in range(2)] for j in range(2)]
        frac_baremu_dn = [[[frac_dn(nubar_e[2][i], nubar_e[3][i], U[1,i_be-1]*mu, mN, m[i_be-1], sigma_bar_dn[j][l])
                            for i in range(idx_in, idx_fn+1)] for l in range(2)] for j in range(2)]
        frac_xmu_dn = [[[[frac_dn(nu_x[2][i], nu_x[3][i], U[1,j]*mu, mN, m[j], sigma_dn[k][l]) for i in range(idx_in, idx_fn+1)]
                         for j in range(3)] for l in range(2)] for k in range(2)]
        frac_barxmu_dn = [[[[frac_dn(nu_x[2][i], nu_x[3][i], U[1,j]*mu, mN, m[j], sigma_bar_dn[k][l]) for i in range(idx_in, idx_fn+1)]
                         for j in range(3)] for l in range(2)] for k in range(2)]

        # Number of neutrinos interacting with the detector at times t w/o magnetic conversion
