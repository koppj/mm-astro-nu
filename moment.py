...
class neutrino_propagator:
    ...
    def __init__(self, d=10*kpc, a_B_coh=0., Bturb=2., outer_scale=0.01, mu_range=np.linspace(0, 4e-13, 20)*mu_b):
        ...
        self.generate_B_field_gal(d=d, a_B_coh=a_B_coh, Bturb=Bturb, outer_scale=outer_scale, mu_range=mu_range)
    ...
    def generate_B_field_gal(self, d=10*kpc, a_B_coh=0., Bturb=2., outer_scale=0.01,
                             mu_range=np.linspace(0, 4e-13, 20)*mu_b,
                             random_B_coh=False, plot=False):
        """Generate a new Galactic B field map with the given parameters
           ...
           Parameters:
               ...
               random_B_coh: if True, pick random B fields for each of the regions
                             in the coherent B field model. Values are chosen from a Gaussian
                             of width 2 muG
        ...
        pitch_angle    = 11.5*np.pi/180.          # pitch angle from https://arxiv.org/abs/0704.0458
        pitch          = np.tan(pitch_angle)
        phi0_table     = np.array([360, 300, 270, 225, 180, 140, 105, 40]) * np.pi/180.
                                                  # azimuthal angles of arm boundaries relative to our x axis,
                                                  # read from fig. 4 in https://arxiv.org/abs/0704.0458
        ...
        phi_los        = np.pi                    # azimuthal angle of line of sight (0 = away from the Galactic center0)
        theta_los      = 0.                       # angle of the line of sight relative to the Galactic plane
        ...
        r_los_table    = np.sqrt(X_los_table[0]**2 + X_los_table[1]**2) # radial coordinate in Galactic plane
        phi_los_table  = np.arctan2(X_los_table[1], X_los_table[0])     # azimuthal coordinate in Galactic plane
        Bcoh_map       = np.zeros((3,N))          # coherent B field in the galactic plane; the first two entries
                                                  #   along axis 0 correspond to the two axes of that plane;
                                                  #   axis 1 corresponds to the l.o.s. coordinate projected
                                                  #   onto the Galactic plane
                    
        # random coherent B fields?
        if random_B_coh:
            B_coh_table = (1 + a_B_coh) * 1e-10 * rnd.normal(scale=2., size=len(B_coh_table))
        ...
        if plot:
            phi_table_plot = np.linspace(0, 2*np.pi, 100)
            ...
            ax1.add_artist(plt.Circle(X_Earth, 0.2, color='black'))
            ...
            ax1.set_xlim(-10,10)
            ax1.set_ylim(-10,10)
            ax1.xaxis.set_ticks(np.arange(-10, 10.1, 1))
            ax1.yaxis.set_ticks(np.arange(-10, 10.1, 1))
            ax1.set_xlabel('x')  # note: this plot is rotated 90 degrees clockwise
            ax1.set_ylabel('y')  #   compared to fig. 4 of https://arxiv.org/abs/0704.0458
            ax1.grid()
...
#            ax2.plot(r_los_table, 1e10*By, label=r'$B_y$ (turb.)', ls='--')
    ...
    def generate_B_field_extragal(self, d=1e6*kpc, d_in_cluster=1e4*kpc, Bturb=2.,
                                  Nbs=1, verbosity=0):
        ...
        B_cluster  = 1     # intracluster magnetic field [\muG]
        B_extragal = 0.005 # intercluster magnetic field [\muG]
...
#                     # faster and more Pythonic, but not really necessary here
        ...
        Bx_c = [Bx[j]*np.sqrt(N)*B_cluster*1e-10/(np.sqrt(np.sum(np.abs(Bx[j])**2)))
                  for j in range(Nbs)]
        ...
        By_c = [By[j]*np.sqrt(N)*B_cluster*1e-10/(np.sqrt(np.sum(np.abs(By[j])**2)))
                  for j in range(Nbs)]
        ...
        Bx = [np.real(np.fft.ifft(Bk1[j])) for j in range(Nbs)]
        Bx = [Bx[j]*np.sqrt(N)*B_extragal*1e-10/(np.sqrt(np.sum(np.abs(Bx[j])**2)))
                for j in range(Nbs)]
        ...
        Bk2 = np.array([[Btemp[j][i] if i <= Nb/2
                         ...
                         for i in range(Nb)] for j in range(Nbs)])
        By = [np.real(np.fft.ifft(Bk2[j])) for j in range(Nbs)]
        By = [By[j]*np.sqrt(N)*B_extragal*1e-10/(np.sqrt(np.sum(np.abs(By[j])**2)))
                for j in range(Nbs)]
        
        d_table = np.concatenate(( np.linspace(0, d_in_cluster, int(Nc/2)), 
            np.linspace(d_in_cluster+0.001*kpc,   d_in_cluster+d,   Nb),
            np.linspace(d_in_cluster+d+0.001*kpc, d_in_cluster+2*d, int(Nc/2)) )) / kpc
...
#            raise ValueError('invalid mu_range')
    ...
    def propagate_sn_neutrinos(self, mu, d=10*kpc, mh='NH',
        ...
        if (a_B_coh != None and Bturb != None and outer_scale != None) or d != self.d:
            self.generate_B_field_gal(d=d, a_B_coh=a_B_coh, Bturb=Bturb,
        ...
        def frac(mean, alph, sigma):
            ...
            if mean == 0:
                ...
                return integrate.quad(lambda x: spec(x, mean, alph) * self.sigma_extract(sigma, x),
        ...
        # event rate at DUNE
        if dn:
            frac_e_dn = np.array([[[frac(nu_e[2][i], nu_e[3][i], self.sigma['DN'][j][l]) for i in range(idx_in, idx_fn+1)] for l in range(2)] for j in range(2)])
            frac_bare_dn = np.array([[[frac(nubar_e[2][i], nubar_e[3][i], self.sigma['bar_DN'][j][l]) for i in range(idx_in, idx_fn+1)] for l in range(2)] for j in range(2)])
            frac_x_dn = np.array([[[frac(nu_x[2][i], nu_x[3][i], self.sigma['DN'][j][l]) for i in range(idx_in, idx_fn+1)] for l in range(2)] for j in range(2)])
            frac_barx_dn = np.array([[[frac(nu_x[2][i], nu_x[3][i], self.sigma['bar_DN'][j][l]) for i in range(idx_in, idx_fn+1)]
                          for l in range(2)] for j in range(2)])

            frac_emu_dn = frac_e_dn * self.P_app(np.linalg.multi_dot([np.transpose(U[:3,:3]), mu, U[:3,:3]])[i_e-1, i_e-1])
            frac_baremu_dn = frac_bare_dn * self.P_app(np.linalg.multi_dot([np.transpose(U[:3,:3]), mu, U[:3,:3]])[i_be-1, i_be-1])
            frac_mumu_dn = frac_x_dn * self.P_app(np.linalg.multi_dot([np.transpose(U[:3,:3]), mu, U[:3,:3]])[i_mu-1, i_mu-1])
            frac_barmumu_dn = frac_barx_dn * self.P_app(np.linalg.multi_dot([np.transpose(U[:3,:3]), mu, U[:3,:3]])[i_bmu-1, i_bmu-1])
            frac_taumu_dn = frac_x_dn * self.P_app(np.linalg.multi_dot([np.transpose(U[:3,:3]), mu, U[:3,:3]])[i_tau-1, i_tau-1])
            frac_bartaumu_dn = frac_barx_dn * self.P_app(np.linalg.multi_dot([np.transpose(U[:3,:3]), mu, U[:3,:3]])[i_btau-1, i_btau-1])
