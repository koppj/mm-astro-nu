...
for i in range(Nl):
    for j in range(Nb):     
        ...
        def chi_square (mu, mh, dn=True, hk=True):
            ...
            if dn:
                ...
                # ax.plot(binning*1e3, np.sum(bins_dn + bins_bar_dn, axis=0),
                #         ds='steps-post', label='$\\mu_\\nu = 0$', color='xkcd:blue')
                # ax.plot(binning*1e3, np.sum(bins_mu_dn + bins_bar_mu_dn, axis=0),
                #         ds='steps-post', label='$\\mu_\\nu = 10^{-13} \\mu_B$', ls='--',
                #         color='xkcd:red')
                ...
                # plt.tight_layout()
                # plt.savefig('DUNE_1e-13mu_b_NO.pdf')
